// Modified version of burlap.behavior.singleagent.planning.stochastic.valueiteration;
 ...
 *
 */
public class ValueIteration extends PlanningIteration implements Planner {
	...
	public void runVI(){
		...
		for(i = 0; i < this.maxIterations; i++){
			...
			for(HashableState sh : states){
				...
				double maxQ = this.performBellmanUpdateOn(sh); // PI performFixedPolicyBellmanUpdateOn
				delta = Math.max(Math.abs(maxQ - v), delta);
				this.totalStatesConsidered++;
			}
			
			this.triggerValueIterationCallback(i, delta);
			// System.out.println("VI Delta: " + delta);
		...
		this.hasRunVI = true;
		System.out.printf("runVI took %d ms\n", System.currentTimeMillis() - start);
	}
	...
	public boolean performReachabilityFrom(State si){
		...
		System.out.printf("VI reachability took %d ms\n", System.currentTimeMillis() - start);
