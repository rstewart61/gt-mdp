// Modified version of package burlap.behavior.singleagent.planning.stochastic.policyiteration;
...
public class PolicyIteration extends PlanningIteration implements Planner {
	...
	public GreedyQPolicy planFromState(State initialState) {
		...
		if(this.performReachabilityFrom(initialState) || !this.hasRunPlanning){
			...
			do{
				...
				this.evaluativePolicy = new GreedyQPolicy(this.getCopyOfValueFunction());
				this.triggerPolicyIterationCallback(iterations, delta);
				// System.out.println("PI Delta: " + delta);
				iterations++;
		...
		System.out.printf("PI planFromState took %d ms\n", System.currentTimeMillis() - start);
	...
	protected double evaluatePolicy(){
		...
		for(i = 0; i < this.maxIterations; i++){
			...
			for(HashableState sh : states){
				...
				double maxQ = this.performFixedPolicyBellmanUpdateOn(sh, this.evaluativePolicy); // VI: performBellmanUpdateOn
				delta = Math.max(Math.abs(maxQ - v), delta);
				this.totalStatesConsidered++;
			}
			
			maxChangeInPolicyEvaluation = Math.max(delta, maxChangeInPolicyEvaluation);
			// System.out.println("PI.VI Delta: " + delta);
			
			if(delta < this.maxEvalDelta){
				i++;
				break; //approximated well enough; stop iterating
			}
			this.triggerValueIterationCallback(i, delta);
		}
	...
	public boolean performReachabilityFrom(State si){
		...
		this.reachableStates = valueFunction.size();
		System.out.printf("PI reachability took %d ms\n", System.currentTimeMillis() - start);
