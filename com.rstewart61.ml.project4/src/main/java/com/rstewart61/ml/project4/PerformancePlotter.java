...
public class PerformancePlotter extends JFrame implements EnvironmentObserver {
	...
	public void writeStepAndEpisodeDataToCSV(String pathAndBaseNameToUse){
		...
		try {
			...
			outEpisode.write("agent,trial,episode,cumulativeReward,averageReward,cumulativeSteps,numSteps,timeNanos\n");
			...
			for(Map.Entry<String, List<Trial>> e : this.agentTrials.entrySet()){
				...
				for(int i = 0; i < trials.size(); i++){
					...
					for(int j = 0; j < trial.totalEpisodes; j++){
						...
						outEpisode.write(","+trial.cpuTimeEpisode.get(j));
						outEpisode.write("\n");
					}
				}
			}
	...
	public void writeEpisodeDataToCSV(String filePath){
		...
		try {
			...
			//create header
			outEpisode.write("agent,trial,episode,cumulativeReward,averageReward,cumulativeSteps,numSteps,cpuTime\n");
	...
	protected class Trial{
		...
		public List<Long> cpuTimeEpisode = new ArrayList<Long>();
		...
		public long startTime = threadBean.getCurrentThreadCpuTime();
		...
		public void setupForNewEpisode(){
			...
			this.cpuTimeEpisode.add(threadBean.getCurrentThreadCpuTime() - startTime);
			// this.startTime = threadBean.getCurrentThreadCpuTime();
